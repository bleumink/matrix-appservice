...
use matrix_sdk::ruma::{OwnedRoomId, OwnedUserId, RoomId, exports::serde_json::json, presence::PresenceState};
use tokio::sync::RwLock;

use crate::appservice::{
    ApplicationServiceInner, Presence,
    device::Device,
    device::DeviceInner,
    error::Error,
    handler::ApplicationServiceReference,
    http_client::{discard_response, parse_response},
    types::{JoinedRoomResponse, Profile},
};
    ...
    fn appservice(&self) -> Result<Arc<ApplicationServiceInner>> {
        match self.appservice.upgrade() {
            ...
            None => Err(Error::UpgradeError(format!(
                "User {} has no parent application service",
                self.id()
            ))),
        }
    }
...
impl User {
    ...
    pub(crate) async fn populate_known_rooms(&self) -> Result<()> {
        ...
        self.appservice()?
            .room_store()
            .populate_known_rooms(&joined_rooms)
            .await?;
    ...
    pub async fn get_joined_rooms(&self) -> Result<Vec<OwnedRoomId>> {
        ...
        let response = self
            .client()?
            .get(url)
            .query(&[("user_id", self.id().to_owned())])
            .send()
            .await?;

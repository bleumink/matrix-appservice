...
use matrix_sdk::ruma::{EventId, OwnedRoomId, OwnedUserId, RoomId, UserId, events::AnySyncTimelineEvent, serde::Raw};
use tokio::sync::RwLock;

use crate::{
    Error, JoinedMembersResponse, MessagesResponse, Result,
    appservice::{
        ApplicationServiceInner, handler::ApplicationServiceReference, http_client::parse_response, user::User,
    },
};
...
impl ApplicationServiceReference for Room {
    fn appservice(&self) -> Result<Arc<ApplicationServiceInner>> {
        match self.appservice.upgrade() {
            ...
            None => Err(Error::UpgradeError(format!(
                "Room {} has no parent application service",
                self.id()
            ))),
        }
    }
...
impl Room {
    pub async fn from_homeserver(appservice: &Arc<ApplicationServiceInner>, room_id: OwnedRoomId) -> Result<Arc<Self>> {
        ...
        let room = Self {
            inner: Arc::new(inner),
            appservice: Arc::downgrade(appservice),
        };
    ...
    pub async fn get_appservice_users(&self) -> Result<Vec<Arc<User>>> {
        ...
        let futures = user_ids.intersection(&joined_members).map(async |mxid| {
            appservice
                .user_store()
                .get(mxid)
                .await
                .ok_or(Error::UserNotFound(mxid.to_owned()))
        });
...
impl ApplicationServiceReference for RoomStore {
    fn appservice(&self) -> Result<Arc<ApplicationServiceInner>> {
        match self.appservice.upgrade() {
            Some(handler) => Ok(handler),
            None => Err(Error::UpgradeError(
                "Room store has no parent application service".to_string(),
            )),
        }
    }
}

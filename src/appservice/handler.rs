...
use axum::Json;
use matrix_sdk::ruma::{
    OwnedRoomId, OwnedTransactionId, RoomId, TransactionId, UserId,
...
use crate::{
    Error, PingResponse, Result,
    appservice::{
        ApplicationServiceInner, EventContext,
        device::Device,
        encryption::OwnedEncryptionSyncChanges,
        event_handler::EventHandlerStore,
        http_client::{Client, parse_response},
        room::{Room, RoomStore},
        transaction::TransactionLog,
        types::{Config, Ping, Transaction},
        user::{User, UserStore},
    },
};
...
impl ApplicationServiceInner {
    pub async fn new(config: Config) -> Result<Arc<Self>> {
        let mxid = UserId::parse(format!(
            "@{}:{}",
            &config.appservice.username, &config.homeserver.server_name
        ))?;
    ...
    pub async fn run(self: &Arc<Self>) -> Result<()> {
        ...
        match bot_user.get_profile().await {
            ...
            Err(_) => {
                ...
                bot_user.set_displayname(&displayname).await?;
                bot_device
                    .register(Some(self.config.appservice.displayname.clone()))
                    .await?;
            }
        }
    ...
    pub async fn handle_transaction(
        ...
        let message = if (events.len(), ephemeral_events.len()) == (0, 0) {
            ...
            format!(
                "Processing {} event(s), {} ephemeral event(s)",
                events.len(),
    ...
    pub async fn handle_event<'a>(&self, event: Raw<AnySyncTimelineEvent>) -> Result<()> {
        ...
        if let Some(handlers) = self.handler_store().get(&extracted.event_type).await {
            let context = EventContext {
                room_id: RoomId::parse(extracted.room_id)?,
                sender: UserId::parse(extracted.sender)?,
            };
    ...
    async fn extract_sync_tasks(
        ...
        for (mxid, device_map) in key_counts {
            for (device_id, algo_map) in device_map {
                let picked_events = to_device_idx
                    .get(&mxid)
                    .and_then(|devices| devices.get(&device_id))
                    .cloned()
                    .unwrap_or_default();

                let picked_fallback_keys = fallback_keys
                    .get(&mxid)
                    .and_then(|devices| devices.get(&device_id))
                    .cloned()
                    .unwrap_or_default();
    ...
    pub async fn init_bot(self: &Arc<Self>) -> Result<()> {
        let mxid = format!(
            "@{}:{}",
            &self.config.appservice.username, &self.config.homeserver.server_name
        );

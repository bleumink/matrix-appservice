...
use matrix_sdk::{
    ...
    crypto::{
        ...
        types::{
            events::room::encrypted::{EncryptedEvent, RoomEncryptedEventContent},
            requests::{AnyOutgoingRequest, OutgoingRequest},
        },
    },
    deserialized_responses::DecryptedRoomEvent,
    ruma::{
        OneTimeKeyAlgorithm, OwnedDeviceId, OwnedUserId, RoomId, UInt,
        api::{
            IncomingResponse, MatrixVersion, SendAccessToken,
            client::{
                keys::{
                    claim_keys::v3::Response as RumaKeysClaimResponse,
                    get_keys::v3::{Request as RumaKeysQueryRequest, Response as RumaKeysQueryResponse},
                    upload_keys::v3::Response as RumaKeysUploadResponse,
                    upload_signatures::v3::Response as RumaUploadSignaturesResponse,
                },
                message::send_message_event::v3::{
                    Request as RumaSendMessageRequest, Response as RumaSendMessageResponse,
                },
                sync::sync_events::DeviceLists,
                to_device::send_event_to_device::v3::{
                    Request as RumaToDeviceRequest, Response as RumaToDeviceResponse,
                },
            },
        },
        assign,
        events::{AnyToDeviceEvent, EventContent, room::message::RoomMessageEventContent},
...
use crate::{
    Error, Result,
    appservice::{
        ApplicationServiceInner, device::Device, handler::ApplicationServiceReference, room::RoomKind, user::User,
    },
};
...
impl Encryption {
    ...
    pub async fn share_room_key(&self, room_id: &RoomId) -> Result<()> {
        ...
        let to_device_requests = self
            .olm()
            .share_room_key(room_id, members.iter().map(|u| u.as_ref()), encryption_settings)
            .await?;
        ...
        for to_device_request in to_device_requests {
            ...
            if let Ok(response) = response.error_for_status() {
                ...
                self.olm()
                    .mark_request_as_sent(&to_device_request.txn_id, &ruma_response)
                    .await?;
            }
        }
    ...
    pub async fn decrypt_event(&self, event: Raw<EncryptedEvent>, room_id: &RoomId) -> Result<DecryptedRoomEvent> {
        ...
        Ok(self
            .olm()
            .decrypt_room_event(&event.cast(), &room_id, &decryption_settings)
            .await?)
    }
    ...
    pub async fn process_outgoing_request(&self, outgoing_request: OutgoingRequest) -> Result<()> {
        match outgoing_request.request() {
            AnyOutgoingRequest::KeysQuery(request) => {
                ...
                self.olm()
                    .mark_request_as_sent(outgoing_request.request_id(), &response)
                    .await?;
            ...
            AnyOutgoingRequest::SignatureUpload(request) => {
                tracing::info!(
                    "Device {} is uploading a signature to the homeserver",
                    &self.device()?.id()
                );
                let http_response = self.send(request.clone()).await?;

...
use axum::{
    Json, Router,
    extract::{Path, Request, State as AppState},
    middleware::Next,
...
use matrix_sdk::ruma::{
    ...
    events::{
        ...
        room::{
            ...
            encryption::StrippedRoomEncryptionEvent,
...
use crate::appservice::{
    event_handler::EventHandlerStore, http_client::Client, room::RoomStore, transaction::TransactionLog,
    user::UserStore,
};
...
impl<S: 'static> ApplicationService<S> {
    pub async fn run(&self) -> Result<()> {
        ...
        let listen_address = SocketAddr::new(
            self.inner.config().appservice.bind_ip,
            self.inner.config().appservice.port,
        );
    ...
    async fn handle_transaction(
        ...
        inner
            .transaction_log()
            .lock_while(txn_id.clone(), || inner.handle_transaction(&txn_id, body))
            .await
    }
...
impl<S> ApplicationService<S> {
        ...
        match event.membership_change(None) {
            MembershipChange::Joined => {
                appservice
                    .inner
                    .room_store()
                    .add_room_member(&context.room_id, event.state_key)
                    .await?;
            }
            MembershipChange::Left => {
                appservice
                    .inner
                    .room_store()
                    .remove_room_member(&context.room_id, &event.state_key)
                    .await?;
            }
            _ => (),
        };
    ...
    async fn on_room_encryption(
        ...
        appservice
            .inner
            .room_store()
            .upgrade_room_encryption(&context.room_id)
            .await?;
    ...
    async fn on_encrypted_message(
        ...
        for user in users {
            if let Some(device) = user.get_device().await {
                let decrypted = device
                    .encryption()
                    .decrypt_event(event.clone().cast(), &context.room_id)
                    .await;
        ...
        Err(Error::DecryptEvent(format!(
            "Unable to decrypt event in room {}",
            context.room_id
        )))
    }
}
